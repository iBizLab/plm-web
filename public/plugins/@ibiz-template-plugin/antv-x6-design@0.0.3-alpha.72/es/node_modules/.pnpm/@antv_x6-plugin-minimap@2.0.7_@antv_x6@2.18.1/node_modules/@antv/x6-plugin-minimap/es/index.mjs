...
import { appendTo, append } from '../../../../../@antv_x6-common@2.0.16/node_modules/@antv/x6-common/es/dom/elem.mjs';
...
import { position, offset } from '../../../../../@antv_x6-common@2.0.16/node_modules/@antv/x6-common/es/dom/position.mjs';
...
class MiniMap extends View {
    ...
    init(graph) {
        this.graph = graph;
        this.updateViewport = debounce(this.updateViewport.bind(this), 0);
        this.container = document.createElement('div');
        addClass(this.container, this.prefixClassName(ClassName.root));
        const graphContainer = document.createElement('div');
        this.container.appendChild(graphContainer);
        this.viewport = document.createElement('div');
        addClass(this.viewport, this.prefixClassName(ClassName.viewport));
        if (this.options.scalable) {
            this.zoomHandle = document.createElement('div');
            addClass(this.zoomHandle, this.prefixClassName(ClassName.zoom));
            appendTo(this.zoomHandle, this.viewport);
        }
        append(this.container, this.viewport);
        css(this.container, {
    ...
    startListening() {
        if (this.scroller) {
            Event.on(this.graphContainer, `scroll${this.getEventNamespace()}`, this.updateViewport);
        }
        else {
    ...
    stopListening() {
        if (this.scroller) {
            Event.off(this.graphContainer, this.getEventNamespace());
        }
        else {
    ...
    updateViewport() {
        ...
        const position$1 = position(this.targetGraph.container);
        const translation = this.targetGraph.translate();
        translation.ty = translation.ty || 0;
        this.geometry = {
            top: position$1.top + origin.y * targetGraphScale.sy + translation.ty,
            left: position$1.left + origin.x * targetGraphScale.sx + translation.tx,
            width: (this.graphContainer.clientWidth * targetGraphScale.sx) /
                sourceGraphScale.sx,
            height: (this.graphContainer.clientHeight * targetGraphScale.sy) /
                sourceGraphScale.sy,
        };
        css(this.viewport, this.geometry);
    }
    startAction(evt) {
    ...
    scrollTo(evt) {
        ...
        ts.ty = ts.ty || 0;
        if (e.offsetX == null) {
            const offset$1 = offset(this.targetGraph.container);
            x = e.pageX - offset$1.left;
            y = e.pageY - offset$1.top;
        }
        else {
            x = e.offsetX;
            y = e.offsetY;
