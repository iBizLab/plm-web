...
import { children, appendTo, remove, append, prepend } from '../../../../../@antv_x6-common@2.0.16/node_modules/@antv/x6-common/es/dom/elem.mjs';
import { Collection } from '../../../../../@antv_x6@2.18.1/node_modules/@antv/x6/es/model/collection.mjs';
import { offset, width, height } from '../../../../../@antv_x6-common@2.0.16/node_modules/@antv/x6-common/es/dom/position.mjs';
import { css } from '../../../../../@antv_x6-common@2.0.16/node_modules/@antv/x6-common/es/dom/css.mjs';
import { call } from '../../../../../@antv_x6-common@2.0.16/node_modules/@antv/x6-common/es/function/function.mjs';
import { Rectangle } from '../../../../../@antv_x6-geometry@2.0.5/node_modules/@antv/x6-geometry/es/rectangle.mjs';
import { Cell } from '../../../../../@antv_x6@2.18.1/node_modules/@antv/x6/es/model/cell.mjs';
import { removeClass, addClass } from '../../../../../@antv_x6-common@2.0.16/node_modules/@antv/x6-common/es/dom/class.mjs';
import { removeAttribute, attr } from '../../../../../@antv_x6-common@2.0.16/node_modules/@antv/x6-common/es/dom/attr.mjs';
...
class SelectionImpl extends View {
    get graph() {
    ...
    get $boxes() {
        return children(this.container, this.boxClassName);
    }
    get handleOptions() {
        return this.options;
    }
    ...
    startSelecting(evt) {
        ...
        if (evt.offsetX != null &&
            ...
            const offset$1 = offset(graphContainer);
            const scrollLeft = graphContainer.scrollLeft;
            const scrollTop = graphContainer.scrollTop;
            x = evt.clientX - offset$1.left + window.pageXOffset + scrollLeft;
            y = evt.clientY - offset$1.top + window.pageYOffset + scrollTop;
        }
        css(this.container, {
            top: y,
            left: x,
    ...
    filter(cells) {
        ...
        return cells.filter((cell) => {
            ...
            if (typeof filter === 'function') {
                return call(filter, this.graph, cell);
            }
            return true;
        });
    }
    stopSelecting(evt) {
        ...
        switch (action) {
            case 'selecting': {
                let width$1 = width(this.container);
                let height$1 = height(this.container);
                const offset$1 = offset(this.container);
                const origin = graph.pageToLocal(offset$1.left, offset$1.top);
                const scale = graph.transform.getScale();
                width$1 /= scale.sx;
                height$1 /= scale.sy;
                const rect = new Rectangle(origin.x, origin.y, width$1, height$1);
    ...
    updateElementPosition(elem, dLeft, dTop) {
        const strLeft = css(elem, 'left');
        const strTop = css(elem, 'top');
        const left = strLeft ? parseFloat(strLeft) : 0;
        const top = strTop ? parseFloat(strTop) : 0;
        css(elem, 'left', left + dLeft);
        css(elem, 'top', top + dTop);
    }
    ...
    adjustSelection(evt) {
        ...
        switch (action) {
            case 'selecting': {
                const data = eventData;
                if (data.moving !== true) {
                    appendTo(this.container, this.graph.container);
                    this.showRubberband();
                    data.moving = true;
                }
                ...
                const left = parseInt(css(this.container, 'left') || '0', 10);
                const top = parseInt(css(this.container, 'top') || '0', 10);
                css(this.container, {
    ...
    destroySelectionBox(cell) {
        ...
        if (this.canShowSelectionBox(cell)) {
            remove(this.container.querySelector(`[data-cell-id="${cell.id}"]`));
    ...
    hide() {
        removeClass(this.container, this.prefixClassName(Private.classNames.rubberband));
        removeClass(this.container, this.prefixClassName(Private.classNames.selected));
    }
    showRubberband() {
        addClass(this.container, this.prefixClassName(Private.classNames.rubberband));
    }
    hideRubberband() {
        removeClass(this.container, this.prefixClassName(Private.classNames.rubberband));
    }
    showSelected() {
        ...
        addClass(this.container, this.prefixClassName(Private.classNames.selected));
    }
    createContainer() {
        this.container = document.createElement('div');
        addClass(this.container, this.prefixClassName(Private.classNames.root));
        if (this.options.className) {
            addClass(this.container, this.options.className);
        }
        this.selectionContainer = document.createElement('div');
        addClass(this.selectionContainer, this.prefixClassName(Private.classNames.inner));
        this.selectionContent = document.createElement('div');
        addClass(this.selectionContent, this.prefixClassName(Private.classNames.content));
        append(this.selectionContainer, this.selectionContent);
        attr(this.selectionContainer, 'data-selection-length', this.collection.length);
        prepend(this.container, this.selectionContainer);
    }
    ...
    updateContainer() {
        ...
        if (boxContent) {
            if (typeof boxContent === 'function') {
                const content = call(boxContent, this.graph, this, this.selectionContent);
                if (content) {
    ...
    createSelectionBox(cell) {
        ...
        if (this.canShowSelectionBox(cell)) {
            ...
            if (view) {
                ...
                addClass(box, `${className}-${cell.isNode() ? 'node' : 'edge'}`);
                attr(box, 'data-cell-id', cell.id);
                css(box, {
                    position: 'absolute',
                    left: bbox.x,
                    top: bbox.y,
    ...
    confirmUpdate() {
        if (this.boxCount) {
            ...
            for (let i = 0, $boxes = this.$boxes, len = $boxes.length; i < len; i += 1) {
                const box = $boxes[i];
                const cellId = attr(box, 'data-cell-id');
                remove(box);
                this.boxCount -= 1;
