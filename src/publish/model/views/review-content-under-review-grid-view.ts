export default {
  enableRowEdit: true,
  xdataControlName: 'grid',
  loadDefault: true,
  deviewCodeName: 'under_review_grid_view',
  deviewId: '89abb3b56002d4ad7c565ae7c2fd679a',
  accUserMode: 2,
  capLanguageRes: {
    lanResTag: 'DE.LNAME.REVIEW_CONTENT',
  },
  caption: '评审内容',
  codeName: 'review_content_under_review_grid_view',
  appDataEntityId: 'plmweb.review_content',
  appViewEngines: [
    {
      engineCat: 'VIEW',
      engineType: 'GridView',
      id: 'engine',
    },
  ],
  appViewLogics: [
    {
      logicTrigger: 'CUSTOM',
      logicType: 'APPUILOGIC',
      builtinAppUILogic: {
        actionAfterWizard: 'DEFAULT',
        batchAddAppViews: [
          {
            openMode: 'POPUPMODAL',
            refMode: 'IDEA',
            refAppViewId: 'plmweb.idea_review_re_idea_view',
          },
        ],
        newDataAppView: {
          openMode: 'POPUPMODAL',
          refAppViewId: 'plmweb.review_content_edit_view',
        },
        enableBatchAdd: true,
        appDataEntityId: 'plmweb.review_content',
        appUILogicRefViews: [
          {
            openMode: 'POPUPMODAL',
            refAppViewId: 'plmweb.review_content_edit_view',
          },
          {
            openMode: 'POPUPMODAL',
            refMode: 'IDEA',
            refAppViewId: 'plmweb.idea_review_re_idea_view',
          },
        ],
        builtinLogic: true,
        logicType: 'PREDEFINED',
        viewLogicType: 'APP_NEWDATA',
        id: '新建数据',
      },
      builtinLogic: true,
      id: 'newdata',
    },
    {
      logicTrigger: 'CUSTOM',
      logicType: 'APPUILOGIC',
      builtinAppUILogic: {
        openDataAppView: {
          refAppViewId: 'plmweb.review_content_redirect_view',
        },
        editMode: true,
        appUILogicRefViews: [
          {
            refAppViewId: 'plmweb.review_content_redirect_view',
          },
        ],
        builtinLogic: true,
        logicType: 'PREDEFINED',
        viewLogicType: 'APP_OPENDATA',
        id: '打开数据',
      },
      builtinLogic: true,
      id: 'opendata',
    },
  ],
  appViewParams: [
    {
      key: 'PROPERTYMAP',
      value:
        'show_identifier:target_identifier,title:target_title,change_type:change_type,change_version:change_version,id:target_id',
      name: 'PROPERTYMAP',
      id: 'propertymap',
    },
  ],
  appViewRefs: [
    {
      openMode: 'POPUPMODAL',
      realOpenMode: 'POPUPMODAL',
      realTitle: '评审内容编辑视图',
      realTitleLanguageRes: {
        lanResTag: 'PAGE.TITLE.REVIEW_CONTENT.EDITVIEW',
      },
      refAppViewId: 'plmweb.review_content_edit_view',
      name: 'NEWDATA',
      id: 'newdata',
    },
    {
      realTitle: '评审内容数据重定向视图',
      realTitleLanguageRes: {
        lanResTag: 'PAGE.TITLE.REVIEW_CONTENT.REDIRECTVIEW',
      },
      refAppViewId: 'plmweb.review_content_redirect_view',
      name: 'EDITDATA',
      id: 'editdata',
    },
    {
      realOpenMode: 'POPUPMODAL',
      realTitle: '计划关联需求多项数据选择视图',
      realTitleLanguageRes: {
        lanResTag: 'PAGE.TITLE.IDEA.OTHERS_RELATION_IDEA_VIEW',
      },
      refAppViewId: 'plmweb.idea_review_re_idea_view',
      name: 'MPICKUPVIEW:IDEA',
      id: 'mpickupview:idea',
    },
    {
      name: 'MPICKUPVIEW:PAGE',
      id: 'mpickupview:page',
    },
  ],
  controls: [
    {
      detoolbarItems: [
        {
          actionLevel: 100,
          noPrivDisplayMode: 2,
          uiactionId: 'program_review_test_case@review_content',
          uiactionTarget: 'NONE',
          valid: true,
          caption: '规划用例',
          itemType: 'DEUIACTION',
          controlLogics: [
            {
              itemName: 'deuiaction2',
              logicTag: 'toolbar',
              logicType: 'SCRIPT',
              scriptCode: "context.principal_type == 'test_case'",
              triggerType: 'ITEMVISIBLE',
              id: 'test_case',
            },
          ],
          sysImage: {
            cssClass: 'fa fa-plus',
            glyph: 'xf067@FontAwesome',
          },
          tooltip: '规划用例',
          showCaption: true,
          showIcon: true,
          id: 'deuiaction2',
        },
        {
          actionLevel: 100,
          noPrivDisplayMode: 2,
          uiactionId: 'program_review_idea@review_content',
          uiactionTarget: 'NONE',
          valid: true,
          caption: '规划需求',
          itemType: 'DEUIACTION',
          controlLogics: [
            {
              itemName: 'deuiaction1',
              logicTag: 'toolbar',
              logicType: 'SCRIPT',
              scriptCode: "context.principal_type == 'idea'",
              triggerType: 'ITEMVISIBLE',
              id: 'idea',
            },
          ],
          sysImage: {
            cssClass: 'fa fa-plus',
            glyph: 'xf067@FontAwesome',
          },
          tooltip: '规划需求',
          showCaption: true,
          showIcon: true,
          id: 'deuiaction1',
        },
        {
          actionLevel: 100,
          noPrivDisplayMode: 2,
          uiactionId: 'program_review_page@review_content',
          uiactionTarget: 'NONE',
          valid: true,
          caption: '规划页面',
          itemType: 'DEUIACTION',
          controlLogics: [
            {
              itemName: 'page',
              logicTag: 'toolbar',
              logicType: 'SCRIPT',
              scriptCode: "context.principal_type == 'page'",
              triggerType: 'ITEMVISIBLE',
              id: 'page',
            },
          ],
          sysImage: {
            cssClass: 'fa fa-plus',
            glyph: 'xf067@FontAwesome',
          },
          tooltip: '规划页面',
          showCaption: true,
          showIcon: true,
          id: 'page',
        },
        {
          actionLevel: 100,
          noPrivDisplayMode: 2,
          uiactionId: 'program_review_work_item@review_content',
          uiactionTarget: 'NONE',
          valid: true,
          caption: '规划工作项',
          itemType: 'DEUIACTION',
          controlLogics: [
            {
              itemName: 'work_item',
              logicTag: 'toolbar',
              logicType: 'SCRIPT',
              scriptCode: "context.principal_type == 'work_item'",
              triggerType: 'ITEMVISIBLE',
              id: 'work_item',
            },
          ],
          sysImage: {
            cssClass: 'fa fa-plus',
            glyph: 'xf067@FontAwesome',
          },
          tooltip: '规划工作项',
          showCaption: true,
          showIcon: true,
          id: 'work_item',
        },
      ],
      toolbarStyle: 'USER',
      xdataControlName: 'grid',
      codeName: 'under_review_grid_view_toolbar',
      controlType: 'TOOLBAR',
      logicName: '评审内容实体表格视图_工具栏',
      appDataEntityId: 'plmweb.review_content',
      controlParam: {
        id: 'toolbar',
      },
      modelId: '68d96d4c87c102957ef3e7dcaeda8a6b',
      modelType: 'PSDETOOLBAR',
      name: 'toolbar',
      id: 'under_review_grid_view_toolbar',
    },
    {
      aggMode: 'NONE',
      columnEnableFilter: 2,
      columnEnableLink: 2,
      groupMode: 'NONE',
      groupStyle: 'DEFAULT',
      degridColumns: [
        {
          clconvertMode: 'NONE',
          dataItemName: 'target_identifier',
          excelCaption: '编号',
          appDEFieldId: 'target_identifier',
          valueType: 'SIMPLE',
          aggMode: 'NONE',
          align: 'LEFT',
          caption: '编号',
          codeName: 'target_identifier',
          columnType: 'DEFGRIDCOLUMN',
          noPrivDisplayMode: 1,
          width: 150,
          widthUnit: 'PX',
          enableSort: true,
          id: 'target_identifier',
        },
        {
          clconvertMode: 'NONE',
          dataItemName: 'target_title',
          excelCaption: '标题',
          appDEFieldId: 'target_title',
          deuiactionGroup: {
            uiactionGroupDetails: [
              {
                actionLevel: 200,
                afterItemType: 'NONE',
                beforeItemType: 'NONE',
                buttonStyle: 'DEFAULT',
                detailType: 'DEUIACTION',
                uiactionId: 'remove_case_all@review_content',
                tooltip: '移出',
                showIcon: true,
                sysImage: {
                  cssClass: 'fa fa-mail-reply',
                  glyph: 'xf112@FontAwesome',
                },
                id: 'u4fb0bbd',
              },
              {
                actionLevel: 200,
                afterItemType: 'NONE',
                beforeItemType: 'NONE',
                buttonStyle: 'DEFAULT',
                detailType: 'DEUIACTION',
                uiactionId: 'review_content_version_comparison@review_content',
                tooltip: '版本比对',
                showIcon: true,
                sysImage: {
                  cssClass: 'fa fa-exchange',
                  glyph: 'xf0ec@FontAwesome',
                },
                id: 'u1a93324',
              },
            ],
            appDataEntityId: 'plmweb.review_content',
            uniqueTag: 'review_content__Usr0428139586',
            name: '评审内容行为组（向导）',
            id: 'usr0428139586',
          },
          valueType: 'SIMPLE',
          aggMode: 'NONE',
          align: 'LEFT',
          caption: '标题',
          codeName: 'target_title',
          columnType: 'DEFGRIDCOLUMN',
          noPrivDisplayMode: 1,
          width: 100,
          widthUnit: 'STAR',
          enableSort: true,
          id: 'target_title',
        },
        {
          clconvertMode: 'FRONT',
          dataItemName: 'change_type',
          excelCaption: '变更类型',
          appCodeListId: 'plmweb.testmgmt__review_change_type',
          appDEFieldId: 'change_type',
          valueType: 'SIMPLE',
          enableRowEdit: true,
          aggMode: 'NONE',
          align: 'LEFT',
          caption: '变更类型',
          codeName: 'change_type',
          columnType: 'DEFGRIDCOLUMN',
          noPrivDisplayMode: 1,
          width: 120,
          widthUnit: 'PX',
          enableSort: true,
          id: 'change_type',
        },
        {
          clconvertMode: 'NONE',
          dataItemName: 'change_version',
          excelCaption: '变更版本',
          appDEFieldId: 'change_version',
          deuiactionId: 'gridview_saverowaction',
          valueType: 'OBJECTS',
          enableRowEdit: true,
          aggMode: 'NONE',
          align: 'LEFT',
          caption: '变更版本',
          codeName: 'change_version',
          columnType: 'DEFGRIDCOLUMN',
          noPrivDisplayMode: 1,
          sysPFPluginId: 'column_change_version',
          width: 184,
          widthUnit: 'PX',
          userParam: {
            'SRFNAVPARAM.n_owner_id_eq': '%owner_id%',
          },
          id: 'change_version',
        },
      ],
      degridDataItems: [
        {
          appDEFieldId: 'target_identifier',
          valueType: 'SIMPLE',
          dataType: 25,
          id: 'target_identifier',
        },
        {
          appDEFieldId: 'target_title',
          valueType: 'SIMPLE',
          dataType: 25,
          id: 'target_title',
        },
        {
          appDEFieldId: 'change_type',
          valueType: 'SIMPLE',
          dataType: 25,
          id: 'change_type',
        },
        {
          appDEFieldId: 'change_version',
          valueType: 'OBJECTS',
          dataType: 21,
          id: 'change_version',
        },
        {
          appDEFieldId: 'target_parent_id',
          valueType: 'SIMPLE',
          dataType: 25,
          id: 'target_parent_id',
        },
        {
          appDEFieldId: 'final_stage_results',
          valueType: 'SIMPLE',
          dataType: 25,
          id: 'final_stage_results',
        },
        {
          appDEFieldId: 'id',
          valueType: 'SIMPLE',
          dataType: 25,
          id: 'srfkey',
        },
        {
          appDEFieldId: 'id',
          valueType: 'SIMPLE',
          dataType: 25,
          id: 'srfdataaccaction',
        },
        {
          appDEFieldId: 'name',
          valueType: 'SIMPLE',
          dataType: 25,
          id: 'srfmajortext',
        },
      ],
      degridEditItems: [
        {
          caption: '变更类型',
          codeName: 'change_type',
          enableCond: 3,
          appDEFieldId: 'change_type',
          editor: {
            singleSelect: true,
            appCodeListId: 'plmweb.testmgmt__review_change_type',
            editorType: 'DROPDOWNLIST',
            valueType: 'SIMPLE',
            editable: true,
            id: 'change_type',
          },
          needCodeListConfig: true,
          id: 'change_type',
        },
        {
          caption: '变更版本',
          codeName: 'change_version',
          enableCond: 3,
          appDEFieldId: 'change_version',
          editor: {
            singleSelect: true,
            handlerType: 'PickupText',
            appDEACModeId: 'default',
            appDEDataSetId: 'fetch_default',
            appDataEntityId: 'plmweb.version',
            enableAC: true,
            forceSelection: true,
            showTrigger: true,
            editorParams: {
              AC: 'TRUE',
              TRIGGER: 'TRUE',
              PICKUPVIEW: 'FALSE',
            },
            editorType: 'PICKEREX_TRIGGER',
            valueType: 'OBJECTS',
            editable: true,
            id: 'change_version',
          },
          allowEmpty: true,
          userParam: {
            'SRFNAVPARAM.n_owner_id_eq': '%owner_id%',
          },
          id: 'change_version',
        },
        {
          caption: '标识',
          codeName: 'srfkey',
          enableCond: 3,
          appDEFieldId: 'id',
          editor: {
            editorType: 'HIDDEN',
            valueType: 'SIMPLE',
            editable: true,
            id: 'srfkey',
          },
          allowEmpty: true,
          id: 'srfkey',
        },
      ],
      pagingSize: 1000,
      sortMode: 'REMOTE',
      enableCustomized: true,
      enableRowEdit: true,
      enableRowNew: true,
      navViewPos: 'NONE',
      createControlAction: {
        appDEMethodId: 'create',
        appDataEntityId: 'plmweb.review_content',
        id: 'create',
      },
      fetchControlAction: {
        appDEMethodId: 'fetch_all',
        appDataEntityId: 'plmweb.review_content',
        id: 'fetch',
      },
      getDraftControlAction: {
        appDEMethodId: 'get_draft',
        appDataEntityId: 'plmweb.review_content',
        id: 'loaddraft',
      },
      getControlAction: {
        appDEMethodId: 'get',
        appDataEntityId: 'plmweb.review_content',
        id: 'load',
      },
      removeControlAction: {
        appDEMethodId: 'remove',
        appDataEntityId: 'plmweb.review_content',
        id: 'remove',
      },
      updateControlAction: {
        appDEMethodId: 'update',
        appDataEntityId: 'plmweb.review_content',
        id: 'update',
      },
      autoLoad: true,
      showBusyIndicator: true,
      controls: [
        {
          detoolbarItems: [
            {
              actionLevel: 100,
              noPrivDisplayMode: 2,
              uiactionId: 'remove_case_all@review_content',
              uiactionTarget: 'MULTIKEY',
              valid: true,
              caption: '移出',
              itemType: 'DEUIACTION',
              sysImage: {
                cssClass: 'fa fa-reply-all',
                glyph: 'xf122@FontAwesome',
              },
              tooltip: '移出',
              showCaption: true,
              showIcon: true,
              id: 'deuiaction2',
            },
          ],
          codeName: 'under_review_grid_view_grid_batchtoolbar',
          controlType: 'TOOLBAR',
          logicName: '批操作工具栏(无权限)',
          appDataEntityId: 'plmweb.review_content',
          controlParam: {
            id: 'grid_batchtoolbar',
          },
          modelId: '0ad1582b07b0daf6e894c631e4362c30',
          modelType: 'PSDETOOLBAR',
          name: 'grid_batchtoolbar',
          id: 'under_review_grid_view_grid_batchtoolbar',
        },
      ],
      codeName: 'under_review_grid_view_grid',
      controlType: 'GRID',
      height: 500,
      logicName: '评审内容实体表格视图_表格',
      appDataEntityId: 'plmweb.review_content',
      controlParam: {
        id: 'grid',
      },
      modelId: '5e0397009813b0a2b6bc1aa0628f9ef8',
      modelType: 'PSDEGRID',
      name: 'grid',
      id: 'plmweb.review_content.under_review_grid_view_grid',
    },
    {
      searchButtonStyle: 'DEFAULT',
      deformPages: [
        {
          layout: {
            columnCount: 24,
            layout: 'TABLE_24COL',
          },
          caption: '常规条件',
          codeName: 'formpage1',
          detailStyle: 'DEFAULT',
          detailType: 'FORMPAGE',
          id: 'formpage1',
        },
      ],
      layout: {
        columnCount: 24,
        layout: 'TABLE_24COL',
      },
      tabHeaderPos: 'TOP',
      noTabHeader: true,
      autoLoad: true,
      showBusyIndicator: true,
      codeName: 'usr04199724_search_form',
      controlType: 'SEARCHFORM',
      logicName: '评审内容实体表格视图_搜索表单',
      appDataEntityId: 'plmweb.review_content',
      controlParam: {
        id: 'searchform',
      },
      modelId: '3de845d7624e39999add4ab7aedcaf0c',
      modelType: 'PSDEFORM_SEARCHFORM',
      name: 'searchform',
      id: 'plmweb.review_content.usr04199724_search_form',
    },
    {
      groupMode: 'SINGLE',
      quickSearchMode: 1,
      enableQuickSearch: true,
      controlType: 'SEARCHBAR',
      appDataEntityId: 'plmweb.review_content',
      controlParam: {
        id: 'searchbar',
      },
      id: 'searchbar',
    },
    {
      capLanguageRes: {
        lanResTag: 'DE.LNAME.REVIEW_CONTENT',
      },
      caption: '评审内容',
      codeName: 'under_review_grid_view_captionbar',
      controlType: 'CAPTIONBAR',
      appDataEntityId: 'plmweb.review_content',
      controlParam: {},
      name: 'captionbar',
      id: 'under_review_grid_view_captionbar',
    },
  ],
  viewLayoutPanel: {
    layoutBodyOnly: true,
    useDefaultLayout: true,
    layoutPanel: true,
    codeName: 'layoutpanel',
    controlStyle: 'APPDEGRIDVIEW',
    controlType: 'VIEWLAYOUTPANEL',
    appDataEntityId: 'plmweb.review_content',
    controlParam: {},
    id: 'layoutpanel',
  },
  title: '评审内容表格',
  viewStyle: 'DEFAULT',
  viewType: 'DEGRIDVIEW',
  enableDP: true,
  showCaptionBar: true,
  modelId: '2d676aa1b73a63a10fd9f26e095d72f1',
  modelType: 'PSAPPDEVIEW',
  name: 'review_contentunder_review_grid_view',
  id: 'plmweb.review_content_under_review_grid_view',
};
